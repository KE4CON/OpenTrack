...
/// can check role level without a database round trip on every request. Shared between
/// OpenTrack.Web (cookie auth) and OpenTrack.API (bearer token auth) so both surfaces
/// enforce identical role checks. Per-project roles (ProjectMembership) are checked
/// separately, closer to the project/issue data.
/// </summary>
